// Interview.jsx — MockHire AI
// ── Features included ──────────────────────────────────────────
// ✅ Resume upload + AI extraction + editable fields
// ✅ Tech / HR / DSA toggle
// ✅ Auto-listen with 3-second countdown
// ✅ Silence detection (25s) + re-ask logic
// ✅ Proctoring: camera PIP + screen share + face detection
// ✅ Violations logged → passed to Feedback page via sessionStorage
// ✅ Feedback fetched here → stored in sessionStorage → Feedback.jsx reads it
// ✅ Ending overlay while AI generates report
...
export default function Interview() {
  ...
  const stopInterview = async () => {
    ...
    // ── Stop proctoring and save violation report ──────────────
    const procReport = getProctoringReport();
    stopProctoring();
    sessionStorage.setItem("proctoringReport", JSON.stringify(procReport));
  ...
  // ══════════════════════════════════════════════════════════════
  //  ENDING OVERLAY — shown while AI generates feedback
  // ══════════════════════════════════════════════════════════════
  if (isEnding) {
    return (
      <div className="iv-page">
        ...
        <div style={{ textAlign:"center", display:"flex", flexDirection:"column", alignItems:"center", gap:24 }}>
          ...
          <h2 style={{ fontSize:28, fontWeight:800, color:"#fff", background:"linear-gradient(135deg,#c8f135,#a78bfa)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>
          ...
          <div style={{ display:"flex", gap:10, marginTop:8 }}>
            {[0,1,2].map(i => (
              <div key={i} style={{ width:10, height:10, borderRadius:"50%", background:"#c8f135", animation:`blink 1.2s ease ${i*0.3}s infinite` }} />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="iv-page">
      ...
      {/* ── Proctoring overlay — camera pip + warnings + screen badge ── */}
      <ProctoringOverlay
        status={procStatus}
        warning={procWarning}
      ...
      <div className="iv-mic-wrap">
        {countdown !== null ? (
          ...
          // Visual-only mic — auto mode, not manually clickable
          <div
